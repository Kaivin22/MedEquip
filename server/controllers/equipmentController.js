...
export async function getAllEquipment(req, res) {
  try {
    ...
    const [rows] = await pool.query("SELECT * FROM thiet_bi WHERE trang_thai = TRUE ORDER BY ngay_tao DESC");
...
export async function deleteEquipment(req, res) {
  try {
    ...
    // Soft-delete: cập nhật trang_thai = FALSE, GIỮ NGUYÊN toàn bộ lịch sử
...
export async function updateEquipment(req, res) {
  try {
    const { tenThietBi, loaiThietBi, donViTinh, moTa, maNhaCungCap, hinhAnh, trangThai } = req.body;
    const id = req.params.id;
    ...
    await pool.query(
      "UPDATE thiet_bi SET ten_thiet_bi = ?, loai_thiet_bi = ?, don_vi_tinh = ?, mo_ta = ?, ma_nha_cung_cap = ?, hinh_anh = ?, trang_thai = ? WHERE ma_thiet_bi = ?",
      [tenThietBi, loaiThietBi, donViTinh, moTa || "", maNhaCungCap || null, hinhAnh || "", newTrangThai, id]
    );
