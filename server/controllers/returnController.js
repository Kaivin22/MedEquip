...
function mapReturn(row, details = []) {
  return {
    ...
    chiTiet: details.map(d => ({
      maPhieuCapPhat: d.ma_phieu_cap_phat,
...
export async function createReturn(req, res) {
  ...
  try {
    ...
    // Tạo phiếu tổng
    await conn.query(
      "INSERT INTO phieu_tra_thiet_bi (ma_phieu_tra, ma_truong_khoa, trang_thai, ghi_chu, qr_data) VALUES (?, ?, 'CHO_XAC_NHAN', ?, ?)",
      [maPhieuTra, userId, ghiChu || "", qrData || maPhieuTra]
    );
    ...
    for (const item of chiTiet) {
      ...
      await conn.query(
        "INSERT INTO chi_tiet_phieu_tra (ma_phieu_tra, ma_phieu_cap_phat, ma_thiet_bi, so_luong, tinh_trang_khi_tra) VALUES (?, ?, ?, ?, ?)",
        [maPhieuTra, item.maPhieuCapPhat, item.maThietBi, item.soLuong, item.tinhTrangKhiTra || "DA_BOC_SEAL"]
      );
...
export async function confirmReturn(req, res) {
  ...
  try {
    ...
    await conn.query("UPDATE phieu_tra_thiet_bi SET trang_thai = ? WHERE ma_phieu_tra = ?", [newStatus, id]);

    // Lấy chi tiết phiếu trả
    const [details] = await conn.query("SELECT * FROM chi_tiet_phieu_tra WHERE ma_phieu_tra = ?", [id]);

    for (const d of details) {
      if (approved) {
        // KIỂM TRA LẠI TRẠNG THÁI CỦA PHIẾU CẤP PHÁT GỐC để phòng TH trùng lặp
        const [cpRows] = await conn.query(
          "SELECT trang_thai_tra FROM phieu_cap_phat WHERE ma_phieu = ?", [d.ma_phieu_cap_phat]
        );
        ...
        if (cpRows.length > 0 && cpRows[0].trang_thai_tra !== 'YEU_CAU_TRA') {
           ...
           console.log(`Bỏ qua mục ${d.ma_phieu_cap_phat} vì trạng thái không hợp lệ: ${cpRows[0].trang_thai_tra}`);
           continue; 
        }
        ...
        await conn.query("UPDATE phieu_cap_phat SET trang_thai_tra = 'DA_TRA' WHERE ma_phieu = ?", [d.ma_phieu_cap_phat]);
      } else {
        // Từ chối: khôi phục trạng thái cũ (CHUA_TRA)
        await conn.query("UPDATE phieu_cap_phat SET trang_thai_tra = 'CHUA_TRA' WHERE ma_phieu = ?", [d.ma_phieu_cap_phat]);
      }
    }
...
export async function cancelReturn(req, res) {
  ...
  try {
    ...
    await conn.query("UPDATE phieu_tra_thiet_bi SET trang_thai = 'HUY' WHERE ma_phieu_tra = ?", [id]);

    // Trả lại trạng thái cho các phiếu cấp phát
    const [details] = await conn.query("SELECT ma_phieu_cap_phat FROM chi_tiet_phieu_tra WHERE ma_phieu_tra = ?", [id]);
    for (const d of details) {
      await conn.query("UPDATE phieu_cap_phat SET trang_thai_tra = 'CHUA_TRA' WHERE ma_phieu = ?", [d.ma_phieu_cap_phat]);
    }
